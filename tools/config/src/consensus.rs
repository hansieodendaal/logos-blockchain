...
use lb_core::{
    block::genesis::{GenesisBlock, GenesisBlockBuilder, MAX_GENESIS_TRANSFER_OUTPUTS},
    mantle::{
...
fn select_sdp_funding_notes_per_node(node_count: usize, additional_wallet_outputs: usize) -> usize {
    ...
    let available_for_sdp = MAX_GENESIS_TRANSFER_OUTPUTS
        .checked_sub(required_outputs)
        .unwrap_or_else(|| {
            panic!(
                "genesis transfer output capacity exhausted: limit={MAX_GENESIS_TRANSFER_OUTPUTS}, node_count={node_count}, fixed_node_outputs={fixed_node_outputs}, additional_wallet_outputs={additional_wallet_outputs}",
            )
        });
    ...
    assert!(
        ...
        "genesis transfer output capacity cannot provide one SDP funding note per node: limit={MAX_GENESIS_TRANSFER_OUTPUTS}, node_count={node_count}, fixed_node_outputs={fixed_node_outputs}, additional_wallet_outputs={additional_wallet_outputs}",
    );
    ...
    assert!(
        projected_genesis_outputs <= MAX_GENESIS_TRANSFER_OUTPUTS,
        "projected genesis transfer outputs exceed capacity: projected={projected_genesis_outputs}, limit={MAX_GENESIS_TRANSFER_OUTPUTS}",
    );

    println!(
        "SDP funding allocation diagnostic=\"blend_tsi_outage\" genesis_transfer_output_limit={MAX_GENESIS_TRANSFER_OUTPUTS} node_count={node_count} fixed_node_outputs={fixed_node_outputs} additional_wallet_outputs={additional_wallet_outputs} requested_sdp_notes_per_node={TARGET_SDP_FUNDING_NOTES_PER_NODE} selected_sdp_notes_per_node={selected_sdp_notes_per_node} sdp_note_value={base_sdp_note_value} sdp_value_remainder={sdp_value_remainder} total_sdp_funding_per_node={SDP_NOTE_VALUE} projected_genesis_outputs={projected_genesis_outputs}",
    );
