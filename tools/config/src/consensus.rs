...
/// Test-tool mirror of the bounded genesis transfer output count.
...
/// many outputs hold that funding.
...
fn select_sdp_funding_notes_per_node(
    ...
    // Each node has one leader and one Blend output before its SDP outputs.
    ...
    let selected = available_for_sdp.min(
        node_count
            .checked_mul(sdp_funding_config.target_notes_per_node)
            .expect("SDP funding output count overflow"),
    ) / node_count;

    assert!(
        selected > 0,
        "genesis transfer output capacity cannot provide one SDP funding note per node"
    );
    selected
}
...
mod tests {
    ...
    #[test]
    fn default_sdp_funding_keeps_one_note_per_node() {
        let ids = [[1; 32], [2; 32]];
    ...
    #[test]
    fn configured_sdp_funding_splits_each_provider_total() {
        let ids = [[1; 32], [2; 32]];
    ...
    #[test]
    fn configured_sdp_funding_caps_notes_to_genesis_capacity() {
