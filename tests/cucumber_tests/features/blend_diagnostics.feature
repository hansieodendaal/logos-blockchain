Feature: Blend diagnostics
  ...
  Scenario Outline: Observe EDGE fork churn after Blend provider reachability outage and partial recovery parameter_set=<parameter_set>
    ...
    # The local manual deployer assigns generated provider identities in its
    # explicit startup order. Start all declared providers before the EDGE
    # population and verify the resulting mapping in the diagnostic step.
    And I start node "NODE_2"
