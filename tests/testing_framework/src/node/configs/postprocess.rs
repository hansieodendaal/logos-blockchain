...
use lb_core::{
    block::genesis::{GenesisBlock, MAX_GENESIS_TRANSFER_OUTPUTS},
    mantle::{Note, ledger::Outputs, traits::GenesisTx as _},
...
fn fit_sdp_funding_outputs_to_genesis_capacity(
    ...
    let available_for_sdp = MAX_GENESIS_TRANSFER_OUTPUTS
        ...
        .unwrap_or_else(|| {
            panic!(
                "genesis transfer output capacity exhausted before SDP funding outputs: limit={MAX_GENESIS_TRANSFER_OUTPUTS}, non_sdp_outputs={non_sdp_outputs}, additional_wallet_outputs={additional_wallet_outputs}",
            )
        });
    ...
    assert!(
        ...
        "genesis transfer output capacity cannot provide one SDP funding note per node: limit={MAX_GENESIS_TRANSFER_OUTPUTS}, node_count={n_participants}, non_sdp_outputs={non_sdp_outputs}, additional_wallet_outputs={additional_wallet_outputs}",
    );
    assert!(
