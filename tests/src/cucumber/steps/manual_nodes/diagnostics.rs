...
pub(super) fn append_timeline_record(world: &CucumberWorld, record: &serde_json::Value) {
    let path = world.scenario_base_dir.join(TIMELINE_FILE);
    let result = (|| -> std::io::Result<()> {
        ...
        if !*header_written {
            ...
            let header = serde_json::json!({
                ...
                "scenario": world.scenario_name.as_deref().unwrap_or("<unknown>"),
            });
...
impl BlendDiagnosticParameterSet {
    fn from_name(name: &str) -> Option<Self> {
        match name {
            // No undue blend behaviour is expected with these parameters when running hapy-path
...
pub async fn observe_epoch_transitions(
    ...
    let initial_observation = query_node_observation(&client).await.map_err(|error| {
...
async fn wait_for_epoch_transitions(
    ...
    loop {
        ...
        let epoch = current_observation.time_info.current_epoch;
        log_due_checkpoints(
            world,
            observation.phase,
            observation.node_name,
            ...
            start_epoch,
            epoch,
            current_observation.time_info.current_slot,
            &current_observation,
        ...
        append_timeline_record(
            ...
            &serde_json::json!({
                ...
                "chain_tip_slot": u64::from(current_observation.consensus.cryptarchia_info.slot),
                "chain_tip_lag_slots": observed_slot.saturating_sub(u64::from(current_observation.consensus.cryptarchia_info.slot)),
                "chain_tip_height": current_observation.consensus.cryptarchia_info.height,
                "chain_tip_id": current_observation.consensus.cryptarchia_info.tip.encode_hex::<String>(),
                "chain_lib_slot": u64::from(current_observation.consensus.cryptarchia_info.lib_slot),
                "chain_lib_id": current_observation.consensus.cryptarchia_info.lib.encode_hex::<String>(),
            }),
        );
...
#[expect(
    ...
    reason = "Transition logging keeps the reference clock and chain fields explicit"
)]
fn log_epoch_transition(
    phase: BlendDiagnosticPhase,
    node_name: &str,
    ...
    observation: &NodeDiagnosticObservation,
) {
    ...
    info!(
        ...
        chain_tip_slot,
        chain_tip_lag_slots = observation.time_info.current_slot.saturating_sub(chain_tip_slot),
...
async fn log_epoch_checkpoint(
    ...
    let mut nodes = world
        .nodes_info
        .iter()
        .filter(|(node_name, _)| !world.blend_diagnostic_stopped_nodes.contains(*node_name))
        .map(|(node_name, node_info)| (node_name.clone(), node_info.started_node.client.clone()))
        .collect::<Vec<_>>();
    nodes.sort_by(|left, right| left.0.cmp(&right.0));

    let mut snapshot_futures = Vec::with_capacity(nodes.len());
    for (node_name, client) in nodes {
        snapshot_futures.push(async move { (node_name, query_node_observation(&client).await) });
    }
    let snapshots = join_all(snapshot_futures).await;
...
fn log_diagnostic_identities(world: &CucumberWorld) -> StepResult {
    ...
    node_names.sort();
    let expected_provider_count = world.blend_core_nodes.unwrap_or(node_names.len());
    ...
    let declared_provider_ids = deployment
        ...
        .genesis_tx()
        .sdp_declarations()
        .filter(|(declaration, _)| declaration.service_type == ServiceType::BlendNetwork)
        .map(|(declaration, _)| declaration.provider_id)
...
pub async fn log_node_lifecycle_marker(
    ...
    let time_info = lifecycle_reference_time(world, event, node_name).await;
...
async fn lifecycle_reference_time(
    world: &CucumberWorld,
    event: &str,
    node_name: &str,
) -> Option<TimeInfo> {
    ...
    match timeout(DIAGNOSTIC_QUERY_TIMEOUT, client.time_info()).await {
        Ok(Ok(time_info)) => Some(time_info),
        Ok(Err(error)) => {
            let error = error.to_string();
            log_reference_query_failure(world, event, node_name, "NODE_2", &error);
            None
        }
        Err(_) => {
            let error = format!(
...
pub async fn log_majority_outage_summary(world: &CucumberWorld) {
    ...
    let time_info = lifecycle_reference_time(world, "node_stop_summary", "NODE_2").await;
    let clock_epoch = time_info.as_ref().map(|time_info| time_info.current_epoch);
    let clock_slot = time_info.as_ref().map(|time_info| time_info.current_slot);
    let timestamp = OffsetDateTime::now_utc();
    info!(
...
mod tests {
    ...
    #[test]
    fn timeline_has_header_and_run_separators() {
        let temp_dir = tempfile::tempdir().expect("temporary directory should be created");
        let mut world = CucumberWorld::default();
        world.scenario_base_dir = temp_dir.path().to_owned();
        ...
        assert_eq!(
            serde_json::from_str::<serde_json::Value>(lines[3])
                .expect("second record should be valid JSON")["event"],
            "second"
        );

        let mut next_run = CucumberWorld::default();
        next_run.scenario_base_dir = temp_dir.path().to_owned();
        next_run.set_scenario_name("next timeline run");
        append_timeline_record(&next_run, &serde_json::json!({"event": "third"}));

        let timeline = fs::read_to_string(temp_dir.path().join(TIMELINE_FILE))
            .expect("timeline should be readable after append");
        let lines: Vec<_> = timeline.lines().collect();
        assert_eq!(lines[4], "");
        let next_header: serde_json::Value =
            serde_json::from_str(lines[5]).expect("next header should be valid JSON");
        assert_eq!(next_header["event"], "blend_diagnostic_timeline_header");
