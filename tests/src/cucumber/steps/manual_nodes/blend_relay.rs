...
use lb_core::{mantle::traits::GenesisTx as _, sdp::ServiceType};
...
use crate::cucumber::{
    ...
    steps::manual_nodes::diagnostics::log_blend_relay_event,
...
impl BlendRelayRegistry {
    ...
    pub fn len(&self) -> Result<usize, io::Error> {
        self.inner
            .lock()
            .map(|inner| inner.relays.len())
            .map_err(|_| io::Error::other("Blend relay registry lock poisoned"))
    }
...
pub async fn set_blend_reachability(
    ...
    let relay_mode_enabled = world
        .blend_diagnostic_relays
        .is_enabled()
        .map_err(|error| {
            relay_step_error(format!("could not inspect Blend relay mode: {error}"))
        })?;
    ...
    if world.blend_diagnostic_stopped_nodes.contains(node_name) {
        return Err(StepError::InvalidArgument {
    ...
    let metadata = world
        .blend_diagnostic_relays
        .metadata(node_name)
        .map_err(|error| relay_step_error(format!("could not inspect Blend relay: {error}")))?
    ...
    if reachable {
        world.blend_diagnostic_unreachable_nodes.remove(node_name);
        if world.blend_diagnostic_phase == Some(BlendDiagnosticPhase::Outage) {
            world.blend_diagnostic_phase = Some(BlendDiagnosticPhase::Recovery);
        }
    } else {
        if world.blend_diagnostic_unreachable_nodes.is_empty() {
            world.blend_diagnostic_phase = Some(BlendDiagnosticPhase::Outage);
        }
        world
...
fn relay_step_error(message: impl Into<String>) -> StepError {
...
#[when("Blend provider endpoints use controllable test relays")]
#[expect(
    clippy::needless_pass_by_ref_mut,
    reason = "Cucumber step functions require a mutable world reference"
)]
fn enable_blend_provider_relay_mode(world: &mut CucumberWorld) -> StepResult {
    if !world.nodes_info.is_empty() {
        return Err(relay_step_error(
            "Blend provider relay mode must be enabled before any node is started",
        ));
    }
    world
        .blend_diagnostic_relays
        .enable()
        .map_err(|error| relay_step_error(format!("failed to enable Blend relays: {error}")))
}

#[when(expr = "I make Blend unreachable on node {string}")]
async fn make_blend_unreachable(world: &mut CucumberWorld, node_name: String) -> StepResult {
    set_blend_reachability(world, &node_name, false).await
}

#[when(expr = "I restore Blend reachability on node {string}")]
async fn restore_blend_reachability(world: &mut CucumberWorld, node_name: String) -> StepResult {
...
mod tests {
    ...
    async fn unused_local_addr() -> SocketAddr {
        ...
        socket
            .local_addr()
            .expect("test UDP socket should have an address")
    }

    async fn relay_fixture() -> (BlendRelay, SocketAddr, SocketAddr, UdpSocket, UdpSocket) {
        let backend = UdpSocket::bind((Ipv4Addr::LOCALHOST, 0))
            .await
            .expect("backend UDP socket should bind");
        let declared_addr = unused_local_addr().await;
        let backend_addr = backend
            .local_addr()
            .expect("backend should have an address");
        let relay = BlendRelay::bind(declared_addr, backend_addr).expect("test relay should bind");
        let client = UdpSocket::bind((Ipv4Addr::LOCALHOST, 0))
            .await
            .expect("client UDP socket should bind");
        (relay, declared_addr, backend_addr, backend, client)
    }

    fn test_run_configs(test_context: &str) -> Vec<RunConfig> {
        let (configs, genesis_block) = create_general_configs(2, Some(test_context));
    ...
    async fn enabled_relay_forwards_client_and_backend_datagrams() {
        let (relay, declared_addr, _backend_addr, backend, client) = relay_fixture().await;

        client
            .send_to(b"client-to-backend", declared_addr)
            .await
        ...
        assert_eq!(source, declared_addr);
    ...
    async fn disabled_relay_blackholes_and_restore_creates_a_fresh_session() {
        ...
        client
            .send_to(b"before-disable", declared_addr)
            .await
        ...
        client
            .send_to(b"while-disabled", declared_addr)
            .await
        ...
        client
            .send_to(b"after-restore", declared_addr)
            .await
    ...
    async fn reconfiguring_provider_reuses_existing_relay() {
        ...
        let conflicting_address = configs
            .pop()
            .expect("second config should be available")
            .user
            .blend
            .core
            .backend
        ...
        assert!(
            registry
                .configure_provider("NODE_1", &mut restarted_config, &conflicting_address)
                .is_err()
        );
